...
def dataset_versioning(
    ...
    with col1[0]:
        task_id = st.text_input(
            label="CVAT Task ID", value="39", key=f"{page_key}_versioning"
        )
    with col1[1]:
    ...
    if is_merging == "True":
        with col1[3]:
            merge_ref = st.selectbox(
                label="Merging Dataset", options=release_tags)
    ...
    if btn:
        ...
        if is_merging == "True":
            ...
            repo = Repository(repo_url=url, ref=merge_ref,
                              token=token, dump_dir=dump_dir)
        else:
            # default to main branch
            repo = Repository(repo_url=url, ref="main",
                              token=token, dump_dir=dump_dir)
        ...
        st.success(
            f'Success Clone Dataset Registry {"/".join(url.split("/")[3:5])}')
        ...
        if is_merging == "True":
            ...
            os.rename(os.path.join(".", repo.repo_dir, "dataset"),
                      os.path.join(".", repo.repo_dir, "old_dataset"))
            fiftyone51.merging_dataset(
                format=annot_type, repo_dir=repo.repo_dir)
